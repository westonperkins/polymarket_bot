...
async def fetch_odds_gamma(
    ...
    if close_client:
        client = httpx.AsyncClient(timeout=httpx.Timeout(_GAMMA_TIMEOUT))
    try:
        resp = await client.get(url, timeout=_GAMMA_TIMEOUT)
        if resp.status_code != 200:
            logger.warning(f"Gamma odds HTTP {resp.status_code}")
            return None
        data = resp.json()
        if not data:
            return None
    ...
    except (httpx.HTTPError, asyncio.TimeoutError, TimeoutError, OSError, ConnectionError) as e:
...
async def fetch_odds_clob(
    ...
    close_client = client is None
    if close_client:
        client = httpx.AsyncClient(timeout=httpx.Timeout(_CLOB_TIMEOUT))
    try:
        resp = await client.get(url, timeout=_CLOB_TIMEOUT)
        if resp.status_code != 200:
            logger.warning(f"CLOB odds HTTP {resp.status_code}")
            return None
        data = resp.json()
...
async def fetch_odds(
    ...
    Retries up to 2 times with 2s delay. Worst case: ~64s (2 rounds x 2 sources x 15s timeout + delays).
    Returns None if all attempts fail (the caller skips the candle gracefully).
    """
    for attempt in range(1, _MAX_RETRIES + 1):
        # Try CLOB first (more real-time)
        odds = await fetch_odds_clob(condition_id, client=client)
        if odds is not None:
            return odds

        # Fall back to Gamma
        logger.info(f"CLOB failed, trying Gamma (attempt {attempt}/{_MAX_RETRIES})")
        odds = await fetch_odds_gamma(slug, client=client)
        if odds is not None:
            return odds
