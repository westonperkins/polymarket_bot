...
async def fetch_odds_gamma(
    ...
    """Fetch current odds from the Gamma API."""
    url = f"{config.POLYMARKET_GAMMA_URL.replace('/markets', '/events')}?slug={slug}"
    close_session = session is None
    if close_session:
        session = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=_GAMMA_TIMEOUT))
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=_GAMMA_TIMEOUT)) as resp:
            if resp.status != 200:
            ...
            return MarketOdds(up_price=up_price, down_price=down_price, spread=spread, tradeable=_is_tradeable(up_price))
...
async def fetch_odds_clob(
    ...
    """Fetch current odds from the CLOB API."""
    url = f"{config.POLYMARKET_CLOB_URL}/markets/{condition_id}"
    close_session = session is None
    if close_session:
        session = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=_CLOB_TIMEOUT))
    try:
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=_CLOB_TIMEOUT)) as resp:
            if resp.status != 200:
            ...
            return MarketOdds(up_price=up_price, down_price=down_price, spread=abs(up_price - down_price), tradeable=_is_tradeable(up_price))
    except (aiohttp.ClientError, asyncio.TimeoutError, TimeoutError, OSError, ConnectionError) as e:
